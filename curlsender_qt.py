...
from PySide6.QtCore import QEvent, QObject, QPoint, QRectF, QSettings, QSize, Qt, QThread, QTimer, Signal, Slot
...
from curlsender_qt_widgets import CodeEditor, FlowLayout, PaintableWidget


_WINDOW_MARGIN = 18
_WINDOW_RADIUS = 30
_CARD_RADIUS = 24
_TITLE_BAR_HEIGHT = 88
_RESIZE_MARGIN = 8
_MIN_WINDOW_SIZE = QSize(1040, 720)
_DEFAULT_WINDOW_SIZE = QSize(1220, 820)
_SETTINGS_ORGANIZATION = "cURLsender"
_SETTINGS_APPLICATION = "ConsoleSignalQt"
_QT_APP_ID = "cURLsender.Qt.V2"
...
_THEMES = {
    "dark": {
        ...
        "dialog_scrim": "#050709",
        "find_match_bg": "#245c61",
        "find_current_bg": "#3db497",
        "find_current_fg": "#071512",
        "divider": "#263139",
        "logo_start": "#3db497",
        "logo_end": "#5f9eff",
    },
    "light": {
        ...
        "dialog_scrim": "#7b7468",
        "find_match_bg": "#d8ecef",
        "find_current_bg": "#135d66",
...
def _format_rate(byte_count: int, seconds: float | None) -> str:
    ...
    while rate >= 1024 and unit_index < len(units) - 1:
        rate /= 1024
        unit_index += 1
    precision = 1 if rate < 10 and unit_index > 0 else 0
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if return_code is not None:
                ...
                self.finished.emit(return_code, elapsed, self._cancel_requested and return_code != 0, total_bytes)
                return
...
class OutputEditor(CodeEditor):
    ...
    def apply_theme(self, colors: dict[str, str]) -> None:
        ...
        self.setStyleSheet(
            (
                "QPlainTextEdit {"
                f"background: {self._colors['output_bg']};"
                f"color: {self._colors['output_fg']};"
                "border: none;"
...
class ExitDialog(QDialog):
    def __init__(self, parent: QWidget, colors: dict[str, str], *, running: bool) -> None:
        ...
        body = QLabel(
            ...
            if running
            else "You can reopen the V2 later from the Qt launcher whenever you want."
        )
...
class ConsoleSignalWindow(QWidget):
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        ring_width = 16
        ring_inset = _WINDOW_MARGIN - ring_width
        outer_radius = _WINDOW_RADIUS + 14
        rect = self.rect().adjusted(ring_inset, ring_inset, -ring_inset, -ring_inset)
    ...
    def _build_ui(self) -> None:
        ...
        self.brand_subtitle = QLabel("Qt V2 shell for faithful curl execution with rounded desktop polish.")
        ...
        for button in (self.theme_btn, self.minimize_btn, self.close_btn):
        ...
        button_row_top.addWidget(self.execute_btn)
        button_row_top.addWidget(self.validate_btn)
        button_row_top.addStretch(1)
        action_layout.addLayout(button_row_top)

        button_row_bottom = QHBoxLayout()
        button_row_bottom.setSpacing(10)
        ...
        button_row_bottom.addWidget(self.clear_output_btn)
        button_row_bottom.addWidget(self.clear_all_btn)
        button_row_bottom.addStretch(1)
        action_layout.addLayout(button_row_bottom)

        for button in (self.execute_btn, self.validate_btn, self.clear_output_btn, self.clear_all_btn):
            button.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        for button in (self.find_btn, self.jump_btn, self.auto_scroll_btn, self.copy_btn):
            button.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        for button in (self.find_prev_btn, self.find_next_btn, self.find_close_btn):
            button.setCursor(Qt.CursorShape.PointingHandCursor)
    ...
    def _build_stylesheet(self) -> str:
        colors = self.colors
        radius = 0 if self.isMaximized() else _WINDOW_RADIUS
        return f"""
        QWidget {{
            color: {colors['text']};
            font: 10pt "Segoe UI";
        ...
        QWidget#Surface {{
            ...
            border: 1px solid {colors['shell_border']};
            border-radius: {radius}px;
        }}
        QFrame[cardRole="default"] {{
            background: {colors['card_bg']};
            border: 1px solid {colors['card_border']};
            border-radius: {_CARD_RADIUS}px;
        }}
        ...
        QFrame[cardRole="metric"][variant="good"] {{
            border-color: {colors['good_fg']};
        }}
        QFrame[cardRole="metric"][variant="warn"] {{
            border-color: {colors['warn_fg']};
        }}
        QFrame[cardRole="metric"][variant="bad"] {{
            border-color: {colors['bad_fg']};
        }}
        QWidget#TitleBar {{
            background: {colors['card_bg']};
            border: 1px solid {colors['card_border']};
        ...
        QLabel#PreflightChip {{
            border-radius: 14px;
            padding: 7px 11px;
            font: 600 9pt "Segoe UI";
        }}
        QLabel#PreflightChip[tone="accent"] {{
            background: {colors['chip_bg']};
            color: {colors['chip_fg']};
        }}
        QLabel#PreflightChip[tone="warn"] {{
            background: {colors['warn_bg']};
            color: {colors['warn_fg']};
        }}
        QLabel#PreflightChip[tone="ghost"] {{
            background: {colors['ghost_bg']};
            color: {colors['muted']};
        }}
        QPushButton {{
        ...
        QPushButton[toolbarRole="true"] {{
            padding: 6px 12px;
            font: 600 9pt "Segoe UI";
            border-radius: 12px;
        }}
        QPushButton[titleRole] {{
            min-width: 34px;
            max-width: 34px;
            min-height: 34px;
            max-height: 34px;
            border-radius: 12px;
            background: {colors['titlebar_btn_bg']};
            color: {colors['titlebar_btn_fg']};
            font: 700 13pt "Segoe UI";
            padding: 0;
        }}
        QPushButton[titleRole]:hover {{
            background: {colors['titlebar_btn_hover']};
        }}
        QPushButton[titleRole="close"]:hover {{
            background: {colors['titlebar_close_hover']};
        }}
        QLineEdit {{
    ...
    def _status_badge_text(self) -> str:
        ...
        if self.command_analysis["has_command"] and not self.command_analysis["valid"]:
            return "Warning"
        if self.command_analysis["valid"]:
            return "Ready"
        return "Idle"

    def _update_status_badge(self) -> None:
        badge_text = self._status_badge_text()
        self.status_badge.setText(badge_text)

        if self.run_state in {"running", "done"}:
            bg = self.colors["good_bg"]
            fg = self.colors["good_fg"]
        elif self.run_state in {"cancelling", "cancelled", "parse error"}:
            bg = self.colors["warn_bg"]
            fg = self.colors["warn_fg"]
        elif self.run_state == "error":
            bg = self.colors["bad_bg"]
            fg = self.colors["bad_fg"]
        else:
    ...
    def _sync_ui_state(self, *, force_defaults: bool = False) -> None:
        if self.run_state == "idle" and force_defaults:
            ...
            elif self.command_analysis["has_command"] and not self.command_analysis["valid"]:
        ...
        if self.run_state == "running":
            ...
            status_value = "Parse error"
        elif self.run_state == "error":
            status_value = "Execution error"
        elif self.run_state == "done":
            status_value = "Done"
        elif self.command_analysis["valid"] and self.command_analysis["has_command"]:
            status_value = "Ready"
        ...
        if self.run_state in {"running", "done"}:
            ...
            status_variant = "bad"
        self.metric_status.set_variant(status_variant)
    ...
    def _refresh_find_matches(self, *_args) -> None:
        ...
        for idx, position in enumerate(positions):
            ...
            if idx == self._find_index:
                selection.format.setBackground(QColor(self.colors["find_current_bg"]))
                selection.format.setForeground(QColor(self.colors["find_current_fg"]))
            else:
                selection.format.setBackground(QColor(self.colors["find_match_bg"]))
            selection.cursor = cursor
            selections.append(selection)

        self.output_editor.set_highlight_selections(selections)
        current_position = positions[self._find_index]
        cursor = self.output_editor.textCursor()
    ...
    def on_execute(self) -> None:
        ...
        if not self.command_analysis["valid"]:
            ...
            line_hint = self.command_analysis["error_line"]
            if line_hint is not None:
                self._append_output(f"Parse error: {error} - unclosed quote opens on line {line_hint}.\n")
            else:
                self._append_output(f"Parse error: {error}\n")
            self.last_exit_code = None
            self.last_elapsed = None
        ...
        args = list(self.command_analysis["args"])
        self._worker_thread = QThread(self)
        self._worker = CurlRunWorker(args)
    ...
    def _handle_window_mouse_event(self, event) -> bool:
        ...
        if event.type() == QEvent.Type.MouseMove:
            ...
            if self._drag_origin is not None and self._drag_start_frame is not None and event.buttons() & Qt.MouseButton.LeftButton:
                offset = global_pos - self._drag_origin
                self.move(self._drag_start_frame + offset)
                return True
    ...
    def _edges_for_global_pos(self, global_pos: QPoint) -> Qt.Edges:
        ...
        surface_rect = self.surface.geometry()
        left = surface_rect.left() <= local.x() <= surface_rect.left() + _RESIZE_MARGIN
        right = surface_rect.right() - _RESIZE_MARGIN <= local.x() <= surface_rect.right()
        top = surface_rect.top() <= local.y() <= surface_rect.top() + _RESIZE_MARGIN
        bottom = surface_rect.bottom() - _RESIZE_MARGIN <= local.y() <= surface_rect.bottom()
    ...
    def _update_resize_cursor(self, global_pos: QPoint) -> None:
        ...
        elif edges == (Qt.Edge.TopEdge | Qt.Edge.LeftEdge) or edges == (
    ...
    def _apply_manual_resize(self, global_pos: QPoint) -> None:
        if self._manual_resize_origin is None:
            return
        geometry = self._manual_resize_geometry
        ...
        if self._manual_resize_edges & Qt.Edge.LeftEdge:
            left += delta.x()
        if self._manual_resize_edges & Qt.Edge.RightEdge:
            right += delta.x()
        if self._manual_resize_edges & Qt.Edge.TopEdge:
            top += delta.y()
        if self._manual_resize_edges & Qt.Edge.BottomEdge:
            bottom += delta.y()

        new_width = max(right - left + 1, self.minimumWidth())
        new_height = max(bottom - top + 1, self.minimumHeight())

        if self._manual_resize_edges & Qt.Edge.LeftEdge:
            left = right - new_width + 1
        if self._manual_resize_edges & Qt.Edge.TopEdge:
            top = bottom - new_height + 1
...
def main() -> int:
    _set_app_user_model_id(_QT_APP_ID)
